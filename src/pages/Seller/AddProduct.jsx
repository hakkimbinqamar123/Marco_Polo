    ...
    return (
            <div className="no-scrollbar flex-1 h-[95vh] overflow-y-scroll flex flex-col justify-between">
                <form onSubmit={onSubmitHandler} className="md:p-10 p-4 space-y-5 max-w-lg">
                        ...
                        <div className="flex flex-wrap items-center gap-3 mt-2">
                            {Array(4).fill('').map((_, index) => (
                                <label key={index} htmlFor={`image${index}`}>
                                    ...
                                    <img className="max-w-24 cursor-pointer" style={{width: "100px", height: "80px"}} src={files[index] ? URL.createObjectURL(files[index]) : "https://cdn3.iconfinder.com/data/icons/photo-tools/65/upload-1024.png"} alt="uploadArea" width={100} height={100} />
                                </label>
                            ))}
                        </div>
                    </div>
                    ...
                    <button  className="px-8 py-2.5 bg-[#e9c810] text-white font-medium rounded cursor-pointer">ADD</button>
                </form>
            </div>
    )
}
